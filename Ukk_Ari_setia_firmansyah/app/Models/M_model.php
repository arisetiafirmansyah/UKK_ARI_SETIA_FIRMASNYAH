    ...
    public function filter2($awal, $akhir)
{
    ...
    $query->select('*');
    $query->join('peminjam', 'peminjaman.peminjam = peminjam.id_peminjam', 'left');
    $query->join('buku', 'peminjaman.buku = buku.id_buku', 'left');

    $query->where('peminjaman.tanggal BETWEEN "' . $awal . '" AND "' . $akhir . '"');
